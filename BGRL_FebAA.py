...
flags.DEFINE_enum('dataset', 'coauthor-cs',
                  ['amazon-computers', 'amazon-photos', 'coauthor-cs', 'coauthor-physics', 'wiki-cs','Actor'],
...
flags.DEFINE_float('drop_ratio_p_1', 0., 'Ratio of node feature dropout 1.')
...
# FebAA
flags.DEFINE_string('LeastVsMost', None, 'Least important or most important features')
flags.DEFINE_string('infOrRandom', None, 'influential or random')
...
def main(argv):
    ...
    dropedges=FLAGS.drop_edge_p_1
    drop_prob = FLAGS.drop_feat_p_1
    percentage = FLAGS.drop_ratio_p_1
    pf = str(drop_prob) + 'x' + str(percentage)  # drop_prob x Percentage
    LvM=FLAGS.LeastVsMost
    dsName=FLAGS.dataset

    inforrandom=FLAGS.infOrRandom
    # prepare transforms
    transform_1 = get_TopGraph(dropedges, data, drop_prob, device, percentage,  dsName, LvM,inforrandom)
    ...
    writer = SummaryWriter(FLAGS.logdir+"_"+LvM+pf)
